...
exports.SpacerAttributeName = void 0;
exports.createLayoutBuilder = createLayoutBuilder;
exports.getBytesPerItem = getBytesPerItem;
...
exports.SpacerAttributeName = SpacerAttributeName;
...
function createLayoutBuilder() {
  ...
  const me = {
    addUint8(name, options) {
      addAttribute(name, _types.AttributeType.Uint8, 1, options);
      return me;
    },

    addUint8Vec2(name, options) {
      addAttribute(name, _types.AttributeType.Uint8, 2, options);
      return me;
    },

    addUint8Vec3(name, options) {
      addAttribute(name, _types.AttributeType.Uint8, 3, options);
      return me;
    },

    addUint8Vec4(name, options) {
      addAttribute(name, _types.AttributeType.Uint8, 4, options);
      return me;
    },

    addFloat32(name, options) {
      addAttribute(name, _types.AttributeType.Float32, 1, options);
      return me;
    },

    addFloat32Vec2(name, options) {
      addAttribute(name, _types.AttributeType.Float32, 2, options);
      return me;
    },

    addFloat32Vec3(name, options) {
      addAttribute(name, _types.AttributeType.Float32, 3, options);
      return me;
    },

    addUint32(name, options) {
      addAttribute(name, _types.AttributeType.Uint32, 1, options);
      return me;
    },
    ...
    build() {
      ...
      toBuild.forEach(attr => {
        if (attr.type === _types.AttributeType.Float32) {
          offset += buildAttribute(attr, offset, built);
        }
      }); // Uint32 next

      toBuild.forEach(attr => {
        if (attr.type === _types.AttributeType.Uint32) {
          offset += buildAttribute(attr, offset, built);
        }
      }); // Uint8 Bytes last

      toBuild.forEach(attr => {
        if (attr.type === _types.AttributeType.Uint8) {
          offset += buildAttribute(attr, offset, built);
        }
      });
      ...
      if (offset % 4 !== 0) {
        offset += buildAttribute({
          ...
          type: _types.AttributeType.Uint8,
          size: 4 - align
        }, offset, built);
      }
...
function getBytesPerItem(layout) {
  ...
  layout.forEach(attribData => {
    byteSizePerItem = Math.max(byteSizePerItem, attribData.offset + attribData.size * (0, _AttributeType.getAttributeTypeByteSize)(attribData.type));
  });
...
function buildAttribute(attr, offset, built) {
  const bytesPerItem = (0, _AttributeType.getAttributeTypeByteSize)(attr.type);
  ...
  for (alias of ((_attr$options2 = attr.options) === null || _attr$options2 === void 0 ? void 0 : _attr$options2.aliases) || []) {
    ...
    built.set(alias.name, {
      ...
      type: alias.type,
      hint: alias.hint,
      typedOffset: offset / (0, _AttributeType.getAttributeTypeByteSize)(alias.type),
      offset
    });
  }
