...
export class VertexSetRenderable extends DataboundRenderable {
    ...
    model;
    nodeGLBuffer;
    _color = new PropertyContainer([160 / 255, 240 / 255, 255 / 255, 207 / 255], areColorsEqual);
    /**
     * Constructor
     ...
     */
    draw(options) {
        const { modelViewMatrix, projectionMatrix, minRadius, maxRadius, weightToPixel, } = options;
        if (this.enabled) {
            this.model.draw({
                parameters: {
                    depthMask: false,
