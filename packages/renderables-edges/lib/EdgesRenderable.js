...
import { processMinMaxBounds, } from '@graspologic/common';
...
export class EdgesRenderable extends DirtyableRenderable {
    ...
    constructor(gl, config, id = getNextId()) {
        ...
        const { model: translucentModel, buffer: translucentModelBuffer, } = createModel(gl, getNextId(), edgeVS, {
            ALPHA_MODE: 1,
        });
     ...
     */
    draw(options) {
        const { modelViewMatrix, projectionMatrix, canvasPixelSize, hideDeselected, framebuffer, engineTime, } = options;
        if (this.enabled) {
     ...
     */
    handleEdgeAttributeUpdated = (storeId, attribute) => {
        this.needsDataBind = true;
        this.setNeedsRedraw(true);
    };
    /**
