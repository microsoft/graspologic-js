...
exports.enablePanZoomEvents = enablePanZoomEvents;
...
function enablePanZoomEvents(renderer, options = DEFAULT_OPTIONS) {
  ...
  function getUIMuliplier() {
    ...
    return Math.abs(position.z) / (rect.width * renderer.camera.projection[0] * 0.5);
  }
  ...
  const eventManager = new _mjolnir.EventManager(view);
  eventManager.on({
    panstart: event => {
      isMouseDown = true;
      lastMouseX = event.x;
    ...
    panmove: event => {
      ...
      if (lastMouseX != null) {
        ...
        if (!renderer.config.is3D || event.srcEvent.shiftKey) {
          ...
          const newRotation = new _math.Quaternion().rotateX(radiansY).rotateY(radiansX);
          renderer.camera.rotation = newRotation.multiply(renderer.camera.rotation, undefined);
        }
    ...
    wheel: event => {
      ...
      if (!Number.isNaN(event.delta)) {
        ...
        renderer.camera.moveTo(position.x, position.y, position.z);
        renderer.makeDirty(); // No need for endUser, it will automatically end
      }
