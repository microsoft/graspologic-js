...
import { AnimationLoop } from '@luma.gl/engine'; // This is causing problems downstream for some reason
// @ts-ignore
...
import { NodesRenderable } from '@graspologic/renderables-nodes'; // typings are messed up for this
...
export class WebGLGraphRenderer extends EventEmitter {
  ...
  constructor(gl, config, data, scene, camera) {
    ...
    this._kickoffDeferred = deferred(); // Pretend all the data has updated
    ...
    this.dimensionInterpolator = new Interpolator(config.interpolationTime); // Start off in the correct position
  ...
  static createInstance(options = {}, data, gl) {
    ...
    const store = data ? createDataStoreFromContainer(data) : createDataStore(options.nodeCountHint, options.edgeCountHint, options.autoBind);
    ...
    const scene = new Scenegraph(gl, config, store); // create nodes renderable

    const nodes = new NodesRenderable(gl, config); // create edges renderable
