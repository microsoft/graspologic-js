 ...
 */
import { Quaternion } from 'math.gl'; // @ts-ignore
...
export function enablePanZoomEvents(renderer, options = DEFAULT_OPTIONS) {
    ...
    panmove: event => {
      ...
      if (lastMouseX != null) {
        ...
        if (!renderer.config.is3D || event.srcEvent.shiftKey) {
          ...
          const newRotation = new Quaternion().rotateX(radiansY).rotateY(radiansX);
