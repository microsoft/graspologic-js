...
import { nodeType, edgeType, } from '@graspologic/graph';
...
export class Scenegraph {
    ...
    *renderables() {
     ...
     */
    get needsRedraw() {
        return (this.doubleBufferedRenderables.needsRedraw ||
    ...
    rebuildSaturation = () => {
        ...
        if (allIn) {
            ...
            for (const prim of this.primitives(undefined, true)) {
                if (prim.type === nodeType) {
                    ;
                    prim.saturation = nodeMap[prim.id || '']
                        ? nodeInSat
                        : nodeOutSat;
                }
                else if (prim.type === edgeType) {
                    ;
                    prim.saturation = !!nodeMap[prim.source]
                        ? edgeInSat
                        : edgeOutSat;
                    prim.saturation2 = !!nodeMap[prim.target]
                        ? edgeInSat
                        : edgeOutSat;
                }
            }
