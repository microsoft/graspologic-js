...
var _TransitioningCameraState = require("./TransitioningCameraState");
...
class Camera extends _common.EventEmitter {
  /**
  ...
  constructor() {
    ...
    this._projection = new _math.Matrix4().perspective(this._projectionSettings); // Make the default view of -1 to 1 in all dimensions

    const defaultState = new _CameraState.CameraState((0, _utils.computeCameraPosition)({
      x: {
        min: -1,
        max: 1
      },
    ...
    }, DEFAULT_FOV, 1.0), new _math.Quaternion());
    this._state = new _TransitioningCameraState.TransitioningCameraState(defaultState.clone(), defaultState.clone(), defaultState.clone(), 0);
  }
  /**
   * Resizes the camera
  ...
  resize(width, height) {
    ...
    this._projection = new _math.Matrix4().perspective(this._projectionSettings);
  }
  /**
  ...
  fitToView(bounds, duration = 0) {
    ...
    const newState = new _CameraState.CameraState((0, _utils.computeCameraPosition)(bounds, this._projectionSettings.fov, this._projectionSettings.aspect), new _math.Quaternion());
  ...
  moveTo(x, y, z, duration = 0) {
    this.transitionToState(new _CameraState.CameraState(new _math.Vector3(x, y, z), this._state.current.rotation.clone()), duration);
  }
  /**
  ...
  computeViewMatrix(rotation = false) {
    let matrix = new _math.Matrix4().translate(this._state.current.position);

    if (rotation) {
      matrix = matrix.multiplyRight(new _math.Matrix4().fromQuaternion(this._state.current.rotation));
    }
  ...
  transitionToState(newState, duration) {
    this._state = new _TransitioningCameraState.TransitioningCameraState(this._state.current.clone(), newState, this._state.current.clone(), duration);
